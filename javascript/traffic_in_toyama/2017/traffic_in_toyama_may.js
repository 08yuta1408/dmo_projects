var traffic_in_toyama_may = [0, 0, 0, 0, 0, 0, 0];      //index番号順に、自家用車、鉄道、高速・路線バス、レンタカー、貸切バス、タクシー、その他
var max_string = 7;
...
$(function(){
    ...
    for(var i = 0; i < data.length; i++){
      ...
      for (var k = 0; k < traffic_array_may.length; k++) {
        if(traffic_array_may[k] == 1){
          traffic_in_toyama_may[k]++;
        }

      }

      // console.log(traffic_array_may);
      // console.log(traffic_in_toyama_may);
    }
  })
  ...
  console.log("----5月----");
  for(var i = 0; i < traffic_in_toyama_may.length; i++){
    console.log(traffic_in_toyama_may[i]);
  }
