var traffic_to_toyama_2017_january = [0, 0, 0, 0, 0, 0, 0, 0];      //index番号順に、飛行機、新幹線、自家用車、JR在来線、高速・路線バス、レンタカー、貸切バス、その他
var max_string = 8;
...
$(function(){
  $.ajax({url: 'json/2017/data_january.json', dataType: 'json', async: false}).done(function(data){
    for(var i = 0; i < data.length; i++){
      //初期化
      traffic_string_2017_january = data[i]["trans home to toyama"];
      ...
      for (var k = 0; k < traffic_array_2017_january.length; k++) {
        if(traffic_array_2017_january[k] == 1){
          traffic_to_toyama_2017_january[k]++;
        }

      }

      // console.log(traffic_array_2017_january);
      // console.log(traffic_to_toyama_2017_january);
    }
  })
