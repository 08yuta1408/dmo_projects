var traffic_to_toyama_march = [0, 0, 0, 0, 0, 0, 0, 0];      //index番号順に、飛行機、新幹線、自家用車、JR在来線、高速・路線バス、レンタカー、貸切バス、その他
var max_string = 8;
...
$(function(){
  $.ajax({url: 'json/2017/data_march.json', dataType: 'json', async: false}).done(function(data){
    for(var i = 0; i < data.length; i++){
      //初期化
      traffic_string_march = data[i]["trans home to toyama"];
      traffic_array_march = [];
      ...
      for (var k = 0; k < traffic_array_march.length; k++) {
        if(traffic_array_march[k] == 1){
          traffic_to_toyama_march[k]++;
        }

      }

      // console.log(traffic_array_march);
      // console.log(traffic_to_toyama_march);
    }
  })
  ...
  console.log("----3月----");
  for(var i = 0; i < traffic_to_toyama_march.length; i++){
    console.log(traffic_to_toyama_march[i]);
  }
