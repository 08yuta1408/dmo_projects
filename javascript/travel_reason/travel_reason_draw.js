$(function(){
  ...
  reason_other = [0, 0, 0, 0, 0, 0];                   //index番号順に、ビジネス、友人・親戚を訪問、予算の関係、北陸新幹線が開業したので、近隣県訪問のついでに、その他

  //集計を行う
  for (var i = 0; i < reason_see.length; i++) {
    reason_see[i] = reason_see_2016_october[i] + reason_see_2016_november[i] + reason_see_2016_december[i] + reason_see_2017_january[i] + reason_see_feb[i] + reason_see_march[i] + reason_see_april[i] + reason_see_may[i] +  reason_see_june[i] + reason_see_2017_july[i] + reason_see_2017_august[i];
  }
  for (var i = 0; i < reason_eat.length; i++) {
    reason_eat[i] = reason_eat_2016_october[i] + reason_eat_2016_november[i] + reason_eat_2016_december[i] + reason_eat_2017_january[i] + reason_eat_feb[i] + reason_eat_march[i] + reason_eat_april[i] + reason_eat_may[i] +  reason_eat_june[i] + reason_eat_2017_july[i] + reason_eat_2017_august[i];
  }
  for (var i = 0; i < reason_play.length; i++) {
    reason_play[i] = reason_play_2016_october[i] + reason_play_2016_november[i] + reason_play_2016_december[i] + reason_play_2017_january[i] + reason_play_feb[i] + reason_play_march[i] + reason_play_april[i] + reason_play_may[i] +  reason_play_june[i] + reason_play_2017_july[i] + reason_play_2017_august[i];
  }
  for (var i = 0; i < reason_buy.length; i++) {
    reason_buy[i] = reason_buy_2016_october[i] + reason_buy_2016_november[i] + reason_buy_2016_december[i] + reason_buy_2017_january[i] + reason_buy_feb[i] + reason_buy_march[i] + reason_buy_april[i] + reason_buy_may[i] +  reason_buy_june[i] + reason_buy_2017_july[i] + reason_buy_2017_august[i];
  }
  for (var i = 0; i < reason_other.length; i++) {
    reason_other[i] = reason_other_2016_october[i] + reason_other_2016_november[i] + reason_other_2016_december[i] + reason_other_2017_january[i] + reason_other_feb[i] + reason_other_march[i] + reason_other_april[i] + reason_other_may[i] +  reason_other_june[i] + reason_other_2017_july[i] + reason_other_2017_august[i];
  }
  ...
  jQuery . jqplot(
    ...
    travel_reason_1 = [ [ reason_other[5], 1 ], [ reason_other[4], 2 ], [ reason_other[3], 3 ], [ reason_other[2], 4 ], [ reason_other[1], 5 ], [ reason_other[0], 6 ], [ reason_buy_feb[2], 7 ], [ reason_buy_feb[1], 8 ], [ reason_buy_feb[0], 9 ], [ reason_play_feb[6], 10], [ reason_play_feb[5], 11],　[ reason_play_feb[4], 12], [ reason_play_feb[3], 13], [ reason_play_feb[2], 14], [ reason_play_feb[1], 15], [ reason_play_feb[0], 16], [reason_eat_feb[11], 17], [reason_eat_feb[10], 18], [reason_eat_feb[9], 19], [reason_eat_feb[8], 20], [reason_eat_feb[7], 21], [reason_eat_feb[6], 22], [reason_eat_feb[5], 23], [reason_eat_feb[4], 24], [reason_eat_feb[3], 25], [reason_eat_feb[2], 26], [reason_eat_feb[1], 27], [reason_eat_feb[0], 28], [reason_see_feb[3], 29], [reason_see_feb[2], 30], [reason_see_feb[1], 31], [reason_see_feb[0], 32] ];
    ...
    yticks = ['その他', '近隣県訪問のついでに', '北陸新幹線が開業したので', '予算の関係', '友人・親戚を訪問', 'ビジネス', '海鮮・生鮮品', '伝統工芸', '街歩き・食べ歩き', '祭りやイベント', '釣り', '登山', '温泉', '各種体験プログラム', 'ドライブ・ツーリング', '工場見学など産業観光', 'ご当地グルメ(ブラックラーメン等)', '日本酒', '川の幸(あゆ)', '山の幸', 'その他海鮮', 'ほたるいか', '白えび', 'ぶり・かに', '寿司', '川の幸(鮎等)', '山の幸', '海鮮', '寺社仏閣参拝', '美術館・博物館', '史跡・文学碑・建造物', '自然の景観'];
